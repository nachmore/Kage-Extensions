/**
 * Dev Tools search provider — extracted from floating-devtools.js.
 * Provides UUID generation, base64 encode/decode, hashing, epoch conversion, JSON formatting.
 */
...
function matchDevTool(input, config) {
    ...
    if (config.base64 && /^(base64d|b64d|decode)\s+.+/i.test(lower)) {
    ...
    if (config.hash && /^(md5|sha1|sha256|sha512)\s+.+/i.test(lower)) {
        const match = input.match(/^(md5|sha1|sha256|sha512)\s+(.+)/i);
        if (match) {
    ...
    if (config.epoch && /^\d{10,13}$/.test(lower)) {
        const ts = lower.length === 13 ? parseInt(lower) : parseInt(lower) * 1000;
    ...
    if (config.epoch && (lower === 'now' || lower === 'epoch' || lower === 'timestamp')) {
        const now = Date.now();
...
function md5(string) {
    ...
    let s = '';
    for(i=0;i<4;i++)for(let j=0;j<4;j++)s+=hex.charAt((state[i]>>(j*8+4))&0xF)+hex.charAt((state[i]>>(j*8))&0xF);
    return s;
}
